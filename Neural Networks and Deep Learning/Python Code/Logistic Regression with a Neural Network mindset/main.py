...
########################################################################################################################
# Reshape the training and test data sets so that images of size (num * px, num * px, 3) are flattened into single vectors of
...
# For a parameter theta, the update rule is theta = theta - alpha * d(theta), where alpha is the learning rate.
########################################################################################################################
...
# (functions implemented in the previous parts) together
...
print("y = " + str(test_set_y[0, 1]) + ", you predicted that it is a \"" +
      classes[int(d["Y_prediction_test"][0,1])].decode("utf-8") + "\" picture.")
...
my_image = "Demo.jpg"  # change this to the name of your image file
## END CODE HERE ##
